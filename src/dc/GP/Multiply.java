...
public class Multiply  extends AbstractNode implements Cloneable {

	public String label = "Multiply";
	...
	public Object deepCopy(Object oldObj) throws Exception {
		ObjectOutputStream oos = null;
	      ...
	      try
	      {
	         ...
	         return (Multiply) ois.readObject(); // G
	      }
	      catch(Exception e)
	      {
	...
	public Multiply clone() {
		AbstractNode  multiply = new Multiply();
		 ...
		 multiply.perfScore =  this.perfScore;
		 ((Multiply)multiply).label = this.label;
		 ((Multiply)multiply).numChildren = this.numChildren;
		 ((Multiply)multiply).parent =  this.parent;
		 ((Multiply)multiply).setNodeIndex(this.nodeIndex);
	     for (int i=0; i<this.numChildren; i++) {
	    	 AbstractNode childAdd = this.children.get(i).clone();
	        	childAdd.setNodeIndex(i);
	        	childAdd.setParent(multiply);
	        	
	    	 ((Multiply)multiply).children.add(childAdd);
	    }
	  //   this.children.clear();
	...
	@Override
	public Multiply cloneAndReplaceLeafNode() {
		AbstractNode  multiply = new Multiply();
	...
	public AbstractNode pruneNode() {
			...
			//		System.out.println("multiple2 return " + value );
				...
				if  (isValueNumeric(node1.getLabel()) == true  &&  isValueNumeric(node2.getLabel()) == true)
				{
					...
					//System.out.println("multiple1 return " + total );
					return  newNode;
