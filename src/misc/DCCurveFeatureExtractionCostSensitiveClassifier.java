...
public class DCCurveFeatureExtractionCostSensitiveClassifier {
	Event[] events;
	Event[] testEvents;
	Event[] GPtestEvents;
	...
	public DCCurveFeatureExtractionCostSensitiveClassifier(Classifier classifier, Classifier classifierNoFilter) {
	...
	try {
		ExtractIndicators.fileWriterTemporalByTime.write(filename + 
				" ," + thresholdString+
		...
		ExtractIndicators.fileWriterTemporalByTime.write("\n");
	} catch (IOException e) {
		// TODO Auto-generated catch block
		e.printStackTrace();
	}
	...
	public void extractDCfeatures() {
		...
		for (int k=0; k<events.length-1; k++){
			...
			String startHour = string[0].replaceFirst("^0+(?!$)", "");
			...
			endCalendar.clear();
			endCalendar.set(Integer.parseInt(endYear), Integer.parseInt(endMonth)-1, Integer.parseInt(endDay),Integer.parseInt(endHour), Integer.parseInt(endMinute), Integer.parseInt(endSeconds));
			...
			startCalendar.clear();
			startCalendar.set(Integer.parseInt(startYear), Integer.parseInt(startMonth)-1, Integer.parseInt(startDay),Integer.parseInt(startHour), Integer.parseInt(startMinute), Integer.parseInt(startSeconds));
			...
			this.events[k].datapoints = new ArrayList<FReader.FileMember2>(FReader.dataRecordInFileArray.subList(events[k].start, events[k].end+1));
	...
	public void printFeatures(String filename, String thresholdString,double split) {	
		...
		//	writer2.write("\n");
			...
			for (int k=trainingCount; k<this.events.length-1; k++){	
				//String feaures = events[k].percentageDeltaPrice +"," + events[k].percentageDeltaDuration + "," + events[k].type +",?"  ;
				//System.out.println(feaures) ;
				writer2.write(getWekaString(k) +"\n");
			}
			writer2.close();
	...
	public void classify(String filename, String thresholdString){
		...
		//			" TPR_no_afterFilter:  "			+ eval.truePositiveRate(1) +
 			...
 			ExtractIndicators.fileWriter.write(filename + 
					" ," + thresholdString+
 ...
 */
	...
	String getWekaString(int k){
		String features;
			...
			date8.setSeconds(date8.getSeconds() + 1);
			if (curDate.before(date1)){
				features = events[k].percentageDeltaPrice +"," + events[k].percentageDeltaDuration + "," + events[k].type +","  + /* percenatageTemporalByTime1+","+ (((double)eventChunksByTime1.size()/(double)this.events.length) * 100.0)+ ","+*/events[k].hasOverShoot ;
			}
			else if (curDate.after(date1) && curDate.before(date2))
				features = events[k].percentageDeltaPrice +"," + events[k].percentageDeltaDuration + "," + events[k].type +"," +  /*percenatageTemporalByTime2+","+ (((double)eventChunksByTime2.size()/(double)this.events.length) * 100.0)+ ","+ */events[k].hasOverShoot ;
			else if (curDate.after(date2) && curDate.before(date3))
				features = events[k].percentageDeltaPrice +"," + events[k].percentageDeltaDuration + "," + events[k].type +"," + /* percenatageTemporalByTime3+","+ (((double)eventChunksByTime3.size()/(double)this.events.length) * 100.0)+ ","+ */events[k].hasOverShoot ;
			else if (curDate.after(date3) && curDate.before(date4))
				features = events[k].percentageDeltaPrice +"," + events[k].percentageDeltaDuration + "," + events[k].type +"," + /*percenatageTemporalByTime4+","+ (((double)eventChunksByTime4.size()/(double)this.events.length) * 100.0)+ ","+ */events[k].hasOverShoot ;
			else if (curDate.after(date4) && curDate.before(date5))
				features = events[k].percentageDeltaPrice +"," + events[k].percentageDeltaDuration + "," + events[k].type +"," + /*percenatageTemporalByTime5+","+ (((double)eventChunksByTime5.size()/(double)this.events.length) * 100.0)+ ","+*/events[k].hasOverShoot ;
			else if (curDate.after(date5) && curDate.before(date6))
				features = events[k].percentageDeltaPrice +"," + events[k].percentageDeltaDuration + "," + events[k].type +"," + /*percenatageTemporalByTime6+","+ (((double)eventChunksByTime6.size()/(double)this.events.length) * 100.0)+ ","+ */events[k].hasOverShoot ;
			else if (curDate.after(date6) && curDate.before(date7))
				features = events[k].percentageDeltaPrice +"," + events[k].percentageDeltaDuration + "," + events[k].type +"," + /*percenatageTemporalByTime7+","+ (((double)eventChunksByTime7.size()/(double)this.events.length) * 100.0)+ ","+ */events[k].hasOverShoot ;
			else
				features = events[k].percentageDeltaPrice +"," + events[k].percentageDeltaDuration + "," + events[k].type +"," + /*percenatageTemporalByTime8+","+ (((double)eventChunksByTime7.size()/(double)this.events.length) * 100.0)+ ","+ */ events[k].hasOverShoot ;;
