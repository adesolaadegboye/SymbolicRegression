...
public class DCCurveOlsen extends DCCurveRegression {
	...
	public void build(Double[] values, double delta, String GPTreeFileName, Event[] events, PreProcess preprocess) {
		...
		predictionWithClassifier = new double[events.length];
		for (int outputIndex = 0; outputIndex < events.length - 1; outputIndex++) {
			predictionWithClassifier[outputIndex] = meanRatio[0];
		}
